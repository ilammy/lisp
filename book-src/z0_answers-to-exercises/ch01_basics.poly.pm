...
Кроме того, окружение ◊ic{env} неизменно в~процессе вычисления элементов последовательности,
поэтому его можно не~включать лишний раз в~список аргументов рекурсивно вызываемой функции.
◊ic{evlis} в~среднем имеет дело со~списками длиной порядка трёх-четырёх элементов.
Убрать бесполезные 20%~вызовов ◊ic{evaluate} и ◊ic{pair?} — вполне неплохая оптимизация.
...
Подобное представление окружений известно как ◊term{гирлянда} ввиду очевидного визуального сходства.
Так используется меньше точечных пар; ценой некоторого замедление поиска и модификации значений переменных.
...
Но в~среднем можно сказать, что разница в~быстродействии будет порядка 5–15~раз ◊cite{itw86}.
...
◊code:lisp{
(begin ◊${◊ii{выражение}_1} ◊${◊ii{выражение}_2})
}
◊(eq)
◊code:lisp{
((lambda (void other) (other))
 ◊${◊ii{выражение}_1}
 (lambda () ◊${◊ii{выражение}_2}) )
}
