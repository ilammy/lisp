...
вдоль и поперёк, а исследования по диагонали всё ещё продолжаются.
В~этой главе
речь пойдёт о~различных вариантах понимания функций и их вызовов.
Мы поговорим
о~том, что называется ◊Lisp1 и ◊Lisp2, а также их различиях, вызываемых
концепцией отдельных пространств имён.
...
особого подхода.
Этот базовый тип объектов имеет личный конструктор:
◊ic{lambda}, и поддерживает как минимум одну операцию: применение функции
к~аргументам.
Вряд~ли можно его описать ещё проще, не~сделав совсем бесполезным.
...
для чего она запрограммирована.
К~примеру, с~помощью функций можно представлять
объекты, имеющие поля и методы ◊cite{ar88}.
В~Scheme функциями представляется
...
несовместимых) вариаций языка.
Изначально ◊LISP~1.5 ◊cite{mae+62} не~имел
понятия объекта-функции.
Реализация была такова, что переменная, функция и
...
◊term{A Permanent VALue}, для глобальных переменных; ◊ic{EXPR},
◊term{EXPRession}, для глобальных функций; ◊ic{MACRO} для макросов.}) списка
...
В~{◊CommonLisp} ◊ic{lambda} — это ключевое слово
...
объекты и единое пространство имён, распространяя понятие ◊term{первого класса}
практически на всё.
Объекты первого класса (полноценные объекты) могут
...
◊section{◊texorpdfstring{◊protect◊Lisp1}{Lisp₁}}◊label{lisp1-2-omega/sect:lisp1}
...
между вычислением элемента на месте ◊term{функции} и вычислением элементов,
находящихся на месте ◊term{параметров}.
Давайте ещё раз посмотрим на
...
◊section{◊texorpdfstring{◊protect◊Lisp2}{Lisp₂}}◊label{lisp1-2-omega/sect:lisp2}
...
является имя глобальной функции.
Это справедливо и для программ из предыдущей
главы.
Мы могли~бы сделать данное ограничение частью языка.
Это~бы не~сильно
...
умел только искать нужную функцию по имени.
Для реализации этой идеи изменим
...
встретить переменную: на месте функции или на месте аргумента.
Мы можем
по-разному обрабатывать один и тот~же идентификатор в~зависимости от его
положения.
Если функции требуют особого подхода, то логичным будет также
выделить для них особое пространство имён.
Очевидно, что легче искать имена
функций в~окружении, где нет имён переменных, которые только мешают.
Интерпретатор легко адаптировать для этого случая.
Нам понадобится окружение
...
окружения.
Так как теперь у~нас два окружения и два вычислителя, то мы назовём
это ◊Lisp2~◊cite{sg93}.
...
окружения: ◊ic{env} и ◊ic{fenv}.
В~остальном новая версия отличается только тем,
что за ◊ic{env} следует (как~тень) ◊ic{fenv}.
Естественно, ещё необходимо
...
элемент формы и обеспечить правильный вызов.
Если мы последуем путём
...
окружение стало компактнее, а значит и поиск в~нём ускорился.
Второе
...
анализа.
Есть только один действительно приятный момент: ◊Lisp2 чуть-чуть
...
функция, надо выполнять лишь один раз: при помещении функции в~окружение.
И~так
...
формы вроде ◊ic{((lookup fn fenv) args)} запрещены).
Для этого следует немного
...
А~потеряли мы возможность
◊emph{вычислить} применяемую функцию.
Рассмотрим выражение ◊ic{(if ◊ii{условие}
(+ 3~4) (* 3~4))}.
В~Scheme можно легко вынести аргументы ◊ic{3} и~◊ic{4} за
скобки: ◊ic{((if ◊ii{условие} + *) 3~4)}.
Это просто, понятно и логично.
Фактически, это тождество.
Но в~◊Lisp2 данная программа вызовет ошибку, так как
