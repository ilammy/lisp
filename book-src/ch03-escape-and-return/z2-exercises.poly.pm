...
(tagbody
  ...
  ◊ii{метка◊sub{1}} ◊ii{выражения◊sub{1}}...
         ...
  ◊ii{метка◊sub{n}} ◊ii{выражения◊sub{n}}...
         ... )
}

Все~◊ii{выражения} могут содержать безусловные переходы~◊nobr{◊ic{(go ◊ii{метка})}}
...
от~которого будут наследоваться подклассы конкретных синтаксических~форм?
...
без необходимости определять их все сразу.
