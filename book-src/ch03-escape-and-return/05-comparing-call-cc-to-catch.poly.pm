...
в~стек вызовов помещается блок кода, чтобы обработать значение, которое вернёт условие ◊nobr{◊ic{if}-формы}.
Аналогично, когда выполняется ◊ic{(evaluate-begin (cdr~(begin-cont-e*~k)) (begin-cont-r~k) (begin-cont-k~k))},
то~текущий блок~◊ic{k} отбрасывается, а~вместо него текущим становится ◊ic{(begin-cont-k~k)}.
...
Когда исполнение покидает блок, то~все продолжения, указывающие на него, становятся недействительными.
...
◊indexC{let/cc}
В~диалекте ◊EuLisp ◊seeCite{pe92} есть специальная форма ◊ic{let/cc} со~следующим синтаксисом:
...
Естественно, каждый из этих вариантов имеет свои преимущества, за~определённую плату.
...
который называется ◊ic{call/ep} (от~◊english{◊term{call with exit procedure}}).
...
В~случае с~◊ic{block}, функционально аналогичный объект необходимо создавать самостоятельно,
