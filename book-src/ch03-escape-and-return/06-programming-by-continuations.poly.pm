...
В~общем, если у~нас есть вычисление ◊ic{(foo (bar))}, то оно выворачивается наизнанку: ◊ic{(new-bar foo)},
где ◊ic{foo} является продолжением, которому ◊ic{new-bar} передаст результат вычислений.
Давайте рассмотрим данное преобразование на~примере многострадального факториала.
...
Если результат равен единице, то~к~ней просто применяется~◊ic{k}.
