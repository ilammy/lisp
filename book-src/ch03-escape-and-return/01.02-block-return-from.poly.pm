...
◊indexR{специальные формы!◊ic{block}}
...
Затраты на поиск можно сократить с~помощью ◊term{лексических~меток}, как их называют в~◊|CommonLisp|.
...
которым пользуются настоящие формы ◊ic{catch} и ◊ic{throw} для обмена информацией о~метках.
