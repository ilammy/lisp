...
Однако он вовсе не~является бесполезным:
...
приводятся приёмы сохранения и использования этих срезов —
...
Получается~◊ic{5}, а~не~ожидаемое значение~◊ic{6}, которое~бы получилось при правильной композиции функций.
...
В~статьях ◊cite{fwfd88,df90,hd90,qs91} предлагается сделать продолжения частичными, поддающимися композиции.
...
а~только их часть вплоть до момента возврата значения.
...
Бабах!
Мы~получили бесконечный цикл, так как ◊ic{foo} оказывается связанной с~◊ic{(begin (+~1~[]) (foo~3))},
...
и~необходимы — то~есть, такую программу нельзя написать проще, не~используя продолжения вовсе.
