...
Указанная ◊ii{реализация-по-умолчанию} будет использоваться,
если при вызове функции не~найдётся более подходящего специализированного метода.
Список аргументов указывается как обычно,
за~исключением того, что один из аргументов является ◊term{дискриминантом};
