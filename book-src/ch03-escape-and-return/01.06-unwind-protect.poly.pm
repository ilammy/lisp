...
◊footnote{
  Аналогичная ситуация с~◊ic{car} и~◊ic{cdr}, которые на~самом деле являются акронимами:
  «◊english{contents of address register}» и~«◊english{contents of decrement register}».
  ◊; TODO: очень косноязычное предложение
  Оригинальная реализация ◊LISP-1.5 для IBM~704 обрабатывала точечные пары в~регистрах.
  Естественно, это не~имеет ничего общего с~текущими реализациями, но~названия приклеились.
}
...
Затем выполняются ◊ii{финальные-формы}, а~уже потом ◊ic{unwind-protect} возвращает значение.
...
даже если ◊ii{форма} оказалась прервана переходом:
...
Как мы уже не~раз говорили, точный смысл управляющих структур определить непросто.
...
Здесь пригодилось~бы строгое, формальное определение её семантики и взаимодействия с~другими конструкциями языка.
...
(Мы~обсудим этот феномен подробнее вместе с~деталями реализации ◊ic{unwind-protect}.)

◊(bigskip)

◊; TODO: ◊indexR{управляюшие формы!выразительность}
Конечно~же существуют и другие управляющие формы.
