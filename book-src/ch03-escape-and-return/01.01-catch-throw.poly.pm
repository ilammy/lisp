...
Это не~совсем пространство ◊emph{имён}, так как метки не~обязательно являются идентификаторами, но вполне похоже на~него по~смыслу.
...
Затем исполнение переходит к~соответствующему продолжению,
а~вместо значения ◊ic{catch} подставляется значение ◊ii{формы} из~◊ic{throw}.

Вернёмся к~примеру с~поиском в~двоичном дереве и перепишем его с~помощью ◊ic{catch} и~◊ic{throw}.
...
Переход в~данном случае идентифицируется значением метки, связанной с~продолжением формы ◊ic{catch}.
...
Такие семантические различия хорошо показывают неточность описания управляющих форм с~помощью естественных языков.
