...
а~сами продолжения сделать полноценными значениями языка.
...
Во-первых, она захватывает продолжения — это форма, где доступно продолжение её~вызова,~◊ii{k}:
...
◊noindent
Теперь, когда мы захватили~◊ii{k}, его нужно как-то использовать.
Передать~◊ii{k} самому себе~же будет неудобно по~ряду причин, поэтому необходим другой подход.
Значение~◊ii{k} будет участвовать в~вычислениях,
...
◊footnote{
  ...
  То~есть с~◊ic{(call/cc list)} никаких проблем нет.
}
которая передаётся ◊ic{call/cc}:

◊code:lisp{
◊cont{◊ii{k}}(call/cc (lambda (k) ...))
}
...
◊indexR{реификация}
◊noindent
Функция ◊ic{call/cc} ◊term{реифицирует} продолжение~◊ii{k} в~полноценный объект (◊english{reified contination}), который становится значением переменной~◊ic{k}.
...
◊code:lisp{
◊cont{◊ii{k}}(call/cc (lambda (k) (+ 1 (k 2)))) ⟹ 2
}
...
◊code:lisp{
◊cont{◊ii{k}}(call/cc (lambda (k) (+ 1 (continue k 2)))) ⟹ 2
}
...
превращает его в~унарную функцию и связывает с~переменной ◊ic{exit}.
...
◊noindent
Здесь реифицированное продолжение~◊ii{k} сохраняется в~переменной~◊ic{k}:
...
В~этом примере продолжение~◊ii{k} используется вне создавшей его формы ◊ic{call/cc}
...
начинается при входе в~форму, а~заканчивается с~последним выражением или первым встреченным переходом.
...
но~нельзя выполнять произвольные переходы вперёд по~коду.
...
◊code:lisp{
◊cont{◊ii{k}}(call/cc ◊${\phi}) → ◊cont{◊ii{k}}(◊${\phi} ◊ii{k})
}

◊noindent
где ◊ii{k} является продолжением вызова ◊ic{call/cc}, а~◊${\phi} — это унарная функция.
Вызов ◊ic{call/cc} лишь превращает~◊ii{k} в~полноценный объект языка, который можно передать как аргумент.
Заметьте, что продолжением вызова ◊${\phi} является всё так~же~◊ii{k},
...
В~некоторых языках ◊ic{call/cc} изымает захватываемое продолжение~◊ii{k} из вычислений:
◊ii{k}~больше не~является своим собственным продолжением,
...
Она поглощает все вычисления вместе с~передаваемым значением, полностью уничтожая информацию.
