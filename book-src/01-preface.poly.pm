...
◊; TODO: значение всего но цену ничего? на английском каламбур работает, а на русском уже нет; и вообще, это вроде цитата Вольтера?
Ходит поговорка, что лисперы знают ценность всего, но не~ведают подлинной цены.
За~элегантные возможности языка иногда приходится платить производительностью,
и не~все программисты осознают наличие подобных компромиссов.
Для сокращения этой пропасти в~понимании языка мы серьёзно углубимся в~детали,
поймём природу языковых конструкций, их происхождение и разновидности,
детально изучим семантику и реализацию многих возможностей Лиспа,
изобретённых за всю его продолжительную историю.
...
Аппликативные языки основаны на идее ◊term{функции} — идее, которую математика веками оттачивала и уточняла.
Они присутствуют практически везде, где присутствуют вычисления,
...
Пользоваться такими инструментами без понимания основного их механизма — комбинации —
это как пытаться выразить мысль с~помощью отдельных слов вместо цельного предложения.
Нескольких заученных фраз достаточно для выживания, однако для полноценной жизни требуется вся~мощь~языка.


◊section*[#:label "pref/sect:audience"]{Аудитория}

Книга будет полезна широкой аудитории специалистов:
...
◊itemize{
  ...
  ◊item{
    ...
    которые желают чётче понимать нюансы и стоимость конструкций языка,
...
Книга основана на курсе лекций, который читается в~магистратуре Университета Пьера и Марии~Кюри;
некоторые части курса также преподаются в~Политехнической школе.

Темы, которые мы будем рассматривать, обычно следуют за вводным курсом аппликативных языков вроде Лиспа, Scheme или~ML,
так как подобные курсы чаще всего заканчиваются детальным разбором изучаемого языка.
Цель этой книги — как можно шире раскрыть тему семантики аппликативных языков и разработки их интерпретаторов и компиляторов.
...
В~отличие от многих других книг, мы пойдём дальше и уделим внимание таким существенным для семейства Лиспов вещам как
рефлексия, интроспекция, динамическая кодогенерация и, конечно~же, макросы.
...
главным вопросом которой является семантика аппликативных языков в~общем и Scheme в~частности.
Исследуя множество реализаций, рассматривая их различные аспекты,
мы узнаем с~максимальной точностью, как строятся лисп-системы.
...
поэтому мы будем уточнять и переопределять различными способами
пространства имён (◊Lisp-1, ◊Lisp-2, и~т.~д.),
...
◊; TODO: 50 страниц? это же бессмысленно в браузере
Главы более-менее независимы, занимают примерно по~50~страниц;
каждая глава имеет список упражнений, ответы к~которым расположены в~конце книги.
Список литературы содержит не~только исторически важные книги,
отслеживающие развитие Лиспа с~1960~года,
...
это не~избавит вас ото всех необходимых усилий.
...
◊; TODO: а также синтаксис ==>
Знак ◊(is) читается: «принимает значение», а знак ◊(eq) означает эквивалентность, «имеет то~же значение, что~и».
...
название которого часто сокращают до~◊(RnRS).
◊seeCite{kcr98}
...
Программы на~Scheme содержат символы, знаки,
◊trnote{
...
Константы почти все типов данных могут быть записаны в~программах непосредственно.
...
◊; - заменять "10000" на "10 000" с тонким неразрывным пробелом
