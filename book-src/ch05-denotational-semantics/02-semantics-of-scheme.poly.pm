...
◊$${
◊math-ic{evaluate}\colon \quad ◊Vset{Программа} \times ◊Vset{Окружение} \times ◊Vset{Продолжение} \times ◊Vset{Память} \to ◊Vset{Значение}
}
...
◊$${
\begin{align*}
◊math-ii{интерпретация}\colon & \quad ◊Vset{Программа} \to ◊Vset{Денотация} \\
...
◊$${
\eta\text{-редукция}\colon
\quad \lambda x . (M\ x) ◊to*{\eta} M
\quad \text{где} \ x \ \text{не~свободна~в} \ M
}
...
◊indexE{[]@◊${◊sem{\quad}} (семантические скобки)}
...
фрагменты программ мы будем заключать в~семантические скобки:
◊${◊sem{\quad}}.
