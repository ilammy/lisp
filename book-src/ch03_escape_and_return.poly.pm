...
приводит к~возврату результата сущности, которая называется ◊term{продолжением}.
В~данной главе разбирается эта идея и её исторические предпосылки.
Мы также
...
продолжений».
Одним из отличий Лиспа от других языков является большое
количество механизмов управления ходом вычислений.
Это в~некотором смысле
превращает данную главу в~каталог ◊cite{moz87}, где представлена тысяча~и~одна
управляющая конструкция.
С~другой стороны, мы не~будем вдаваться в~подробности
о~продолжениях; по крайней мере, о~том, как физически реализуется их захват и
сохранение.
Наш интерпретатор будет использовать объекты для представления
продолжений в~виде ◊term{стека вызовов}.
...
К~сожалению, они не~в~состоянии
проводить вычисления, в~которых есть ◊term{переходы} (escapes): полезная
управляющая конструкция, позволяющая покинуть текущий контекст исполнения, чтобы
перейти в~другой, более подходящий.
Обычно они используются для обработки
...
считалось, что для них было более привычным использование ◊ic{goto}.
Вместо
...
переменные (в~данном случае это~◊ic{r}).
Далее следуют инструкции
(представляемые списками) и метки (представляемые символами).
Инструкции
последовательно вычисляются, как в~◊ic{progn}.
Результатом вычисления формы
◊ic{prog} по умолчанию является ◊ic{nil}.
Но внутри ◊ic{prog} можно использовать
специальные инструкции.
Безусловные переходы выполняются с~помощью ◊ic{go}
(которая принимает символ~— имя метки), а вернуть определённое значение из
◊ic{prog} можно с~помощью ◊ic{return}.
В~◊LISP~1.5 было лишь одно ограничение:
...
вычислений.
Ограничение ◊LISP~1.5 допускало лишь простые переходы, в~более
...
в~безопасный для обработки возникшей ситуации.
Теперь можно переписать факториал
...
значение,~— она сама связывает ◊ic{return} с~этим местом.
Для перехода
не~требуется знать, откуда мы уходим, но необходимо знать, куда мы хотим
попасть.

Если такой ◊term{прыжок} будет эффективно реализован, то это порождает
жизнеспособную парадигму программирования.
Например, пусть стоит задача
проверить вхождение элемента в~двоичное дерево.
В~лоб эта задача решается
...
◊indexR{исключения}
Другим примером может быть так называемое программирование исключениями.
Суть
подхода: пусть в~цикле выполняются какие-то действия.
Данный цикл продолжает
...
но и нечто, куда необходимо вернуть полученный результат.
Это нечто и называется
◊term{продолжением} (continuation).
Это всё, что ещё осталось вычислить.

У~любого вычисления есть продолжение.
Например, в~выражении ◊ic{(+~3 (*~2~4))}
продолжением подвыражения ◊ic{(*~2~4)} будет сложение, где первый аргумент
это~◊ic{3}, а второй ожидается в~результате вычислений.
...
требуют, чтобы сначала были вычислены все необходимые параметры.
Для предыдущего
...
Действительно, у~всего есть продолжение.
Вычисление условного выражения в~формах
ветвления проводится для продолжения, которое ожидает это значение, чтобы
выбрать ту или иную ветку условной формы.
В~выражении ◊ic{(if (foo) 1 2)}
...
манипуляции продолжениями.
Имея это в~виду, давайте рассмотрим в~деталях
