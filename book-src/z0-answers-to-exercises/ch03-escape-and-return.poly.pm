...
Итак, вычисляемое выражение: ◊nobr{◊ic{◊cont{◊${k_0}}(cc◊sub{1} cc◊sub{2})}},
где~◊${k_0} — продолжение этого выражения.
Вспомните определение ◊ic{call/cc}:

◊; TODO: форматирование ◊cont{◊${k...}}, это должно превратиться в ◊${{}_{k...}}, как нибудь
◊; особенно так, чтобы в ◊cont{◊${k_0}} k было матемтическим курсивом, а 0 прямым
◊; может заменишь k_0 на что-то другое? потому что двухэтажные подстрочные индексы выглядят не оч
◊code:lisp{
◊cont{◊${k}}(call/cc ◊${\phi}) ◊(is) ◊cont{◊${k}}(◊${\phi} ◊${k})
}

◊noindent
Следовательно, ◊nobr{◊ic{◊cont{◊${k_0}}(cc◊sub{1} cc◊sub{2})}} становится ◊nobr{◊ic{◊cont{◊${k_0}}(cc◊sub{2} ◊${k_0})}},
что в~свою очередь переходит в~◊nobr{◊ic{◊cont{◊${k_0}}(◊${k_0} ◊${k_0})}},
значением которого очевидно является~◊${k_0}.


◊exanswer{escape/ex:cc-cc-cc-cc}

◊; TODO: я хочу, чтобы эти штуки выглядели как выделенные математические формулы
◊; ну или хоть как-нибудь, чтобы не как нечитабельное говно
Используя нотацию предыдущего упражнения, запишем:

◊nobr{◊ic{◊cont{◊${k_0}}((cc◊sub{1} cc◊sub{2}) (cc◊sub{3} cc◊sub{4}))}}

Для простоты будем считать, что термы аппликаций вычисляются слева направо.
Тогда исходное выражение эквивалентно

◊nobr{◊ic{◊cont{◊${k_0}}(◊cont{◊${k_1}}(cc◊sub{1} cc◊sub{2}) (cc◊sub{3} cc◊sub{4}))}}

где~◊${k_1} это ◊${\lambda\phi . ◊text{◊ic{◊cont{◊${k_0}}(◊${\phi} ◊cont{◊${k_2}}(cc◊sub{3} cc◊sub{4}))}}},
а~◊${k_2} соответственно ◊${\lambda\epsilon . ◊text{◊ic{◊cont{◊${k_0}}(◊${k_1} ◊${\epsilon})}}}.

Вычисление первого терма приводит к~◊nobr{◊ic{◊cont{◊${k_0}}(◊${k_1} ◊${k_2})}},
а~вычисление второго даёт ◊nobr{◊ic{◊cont{◊${k_0}}(◊${k_2} ◊cont{◊${k'_2}}(cc◊sub{3} cc◊sub{4}))}},
где~◊${k'_2} соответствует ◊${\lambda\epsilon . ◊text{◊ic{◊cont{◊${k_0}}(◊${k_2} ◊${\epsilon})}}}.

Эта~форма вычисляется в~◊nobr{◊ic{◊cont{◊${k_0}}(◊${k_1} ◊${k'_2})}},
что в~свою очередь даёт ◊nobr{◊ic{◊cont{◊${k_0}}(◊${k'_1} ◊${k''_2})}},
и~так~далее.
...
Если вспомнить, что ◊ic{letrec} эквивалентна комбинации ◊ic{let} и ◊ic{set!},
