...
повторениями и вариациями чем-то напоминают «Болеро» Мориса~Равеля.
Но один
мотив, как вы могли заметить, в~них отсутствовал: присваивание и побочные
эффекты.
Чистые языки их презирают из-за некоторых скверных особенностей, но
поскольку в~диалектах Лиспа их существование допускается, нам следует их
изучить.
В~этой главе подробно рассматривается присваивание, а также прочие
побочные эффекты.
По мере нашего продвижения мы также будем вынуждены
...
в~той или иной мере изменить значение, связанное с~переменной.
Это приводит
к~необходимости сохранять состояние программы (в~том или ином виде), где будет
записано, что такая-то переменная отныне имеет следующее значение.
Для свободно
...
свойств.
То~есть определить язык с~присваиванием и побочными эффектами,
не~используя для этого присваивание и побочные эффекты.
Не~то, чтобы мы их до
...
Присваивание, как мы сказали, позволяет изменить значение переменной.
Например,
...
наибольшего элемента из уже просмотренных.
...
функциями, не~вызывает существенных проблем.
Например, следующая функция
последовательно возвращает натуральные числа, начиная с~нуля.
Очевидно, что
...
Каждый вызов ◊ic{enumerate} возвращает новое натуральное число.
Функция
◊ic{enumerate} имеет внутреннее состояние, представляемое переменной~◊ic{n},
которая изменяется с~каждым следующим вызовом функции.
Но то, что ◊ic{n}
принадлежит замыканию, является проблемой.
В~◊${◊lambda}-исчислении нет ни~слова
о~присваивании.
Ведь когда мы применяем функцию в~математике (и,~соответственно,
в~◊${◊lambda}-исчислении), то просто заменяем в~её теле все переменные
соответствующими значениями аргументов.
Из-за присваивания данная семантика
...
с~мгновенной подстановкой значений.
Теперь подстановка является отложенной и
выполняется только тогда, когда мы явно потребуем значение.
Соответственно,
определённым образом расставленные присваивания могут изменить это значение до
подстановки.

Но с~расположением присваиваний есть свои проблемы.
...
И~снова ◊${◊lambda}-исчисление хранит молчание.
У~него есть причины быть
немногословным: ведь присваивание чуждо ему.
Ладно, мы вроде~бы говорили, что
...
◊ic{name} имела значение ◊ic{"Nemo"}.
Также кажется очевидным, что
...
значение.
Проблема в~том, а видит~ли ◊ic{winner} это значение? Ведь мы изменяем
...
В~таком случае предыдущая программа эквивалентна
следующей:
...
кто это присваивание совершил.
Тогда действительно ◊ic{set-winner!} вернёт новое
значение, а ◊ic{winner} всегда будет возвращать ◊ic{"Nemo"}.
В~таком мире
не~существует привязок.
Нет, конечно у~переменных есть значения, но они намертво
установлены окружением.
Если кому-то нужно значение переменной, то он может
запросить его у~окружения.
Если~же это значение надо изменить, то просто взамен
...
исключительно динамических диалектах Лиспа.
Там замыкания создавались явным
...
функция.
В~результате вызов ◊ic{(closure (x) (lambda (y) (+~x y)))} раскрывался
в~◊ic{(lambda (y) (let ((x~'◊ii{значение-◊ic{x}})) (+~x~y)))}.
С~одной стороны,
...
Scheme решает проблему иным путём, вводя понятие ◊term{привязок}, что также
приводит к~интересным ◊emph{побочным эффектам}.
Форма~◊ic{let} привязывает
к~переменной ◊ic{name} значение ◊ic{"Nemo"}.
Замыкания, создаваемые в~теле
◊ic{let}, захватывают не~значение переменной ◊ic{name}, а её привязку.
Таким
...
хранимое значение.
В~таком случае привязки являются объектами второго класса,
которые не~существуют отдельно от своих переменных.
...
значение».
Они представляются точечными парами.
Когда нам требуется переменная,
мы ищем соответствующую точечную пару.
В~случае присваивания эта пара (точнее,
её ◊ic{cdr}) изменяет своё значение.
Поэтому в~данном случае именно точечные
пары являются воплощением привязок.
Но это не~единственный способ их
представления; например, привязки можно представлять с~помощью ◊term{коробок}
(boxes, также известны как cells).
Новый уровень абстракции имеет большое
...
Значение упаковывается в~коробку с~помощью функции ◊ic{make-box}.
Можно
взглянуть на то, что в~коробке, с~помощью ◊ic{box-ref} и положить в~неё
что-то другое с~помощью ◊ic{box-set!}.
Набросаем реализацию коробок в~стиле
передачи сообщений:
...
возвращаемого значения вызова ◊ic{set-box!}.
Каждую переменную, значение которой
...
коробок.
Заодно мы приобретаем возможность определить, изменяема~ли переменная;
для этого достаточно посмотреть на определяющую её связывающую форму и
убедиться, что переменная упакована в~коробку.
...
◊begin{tabular}{rcl}
  ...
  ◊bc{(begin ◊p{1} ...
◊p{n})} & ◊EQ & ◊ic{(begin ◊b{◊p{1}} ...
◊b{◊p{n}})}   ◊◊
            ◊bc{(set! ◊${x} ◊P)} & ◊EQ & если ◊${x = v},
                                       то ◊ic{(box-set! ◊${v} ◊b◊P)},           ◊◊
                               &     & иначе ◊ic{(set! ◊${x} ◊b◊P)}             ◊◊
◊bc{(lambda (...
◊${x} ...) ◊P)} & ◊EQ & если ◊${v ◊in ◊{{◊dots}◊,◊ x◊ ◊dots◊}},  ◊◊
                               &     & то ◊ic{(lambda (...
◊${x} ...) ◊P)},     ◊◊
                               &     & иначе ◊ic{(lambda (...
◊${x} ...) ◊b◊P)} ◊◊
  ◊bc{(◊p{0} ◊p{1} ...
◊p{n})} & ◊EQ & ◊ic{(◊b{◊p{0}} ◊b{◊p{1}} ...
◊b{◊p{n}})}
...
в~«коробочный~стиль».
Запись~◊boxit{◊${◊pi}} означает, что переменная~◊${v}
в~программе~◊${◊pi} помещается в~коробку.
Как обычно, за именами переменных важно
...
◊code:lisp{
(lambda (...
|◊x| ...) |◊p|) |◊${◊land}| (set! |◊x| ...)|◊${{} ◊in ◊pi}|
|◊is| (lambda (...
|◊x| ...) (let ((|◊x| (make-box |◊x|))) |◊boxit[x]{◊p}|))
}}
...
Благодаря коробкам не~требуется введение специальных
...
переменные ссылаются.
Конечно, изменение значения коробок требует побочных
эффектов, так что ссылочная прозрачность всё так~же остаётся утерянной.
Тем
не~менее, это не~мешает ввести систему типов содержимого коробок.
С~другой
...
можно передавать не~только в~◊ic{box-ref} и ◊ic{box-set!}.
Например, можно
скопировать привязку и дать ей альтернативное имя; это может быть и полезным,
например, при создании модулей.

Во-вторых, мы никак не~можем контролировать использование коробок.
Переменные
теперь можно изменять в~общем случае где угодно, передав копию коробки под
другим именем.
С~лексическим присваиванием такой трюк не~работает: для изменения
значения переменной необходимо статически указать её правильное имя.
Это,
...
со~своим положением в~памяти (адресом), где хранится её значение.
При
...
Переменная ◊ic{can-access?} является свободной для этой формы.
Более того, ей
присваивается значение.
По правилам Scheme ◊ic{can-access?} должна быть
глобальной, так как её нет в~локальных связывающих формах.
Но ведь то, что она
...
значение, определять перед использованием.
Глобальное окружение переменных тоже
