...
красе.
Мы рассмотрим ещё одно определение Лиспа — в~этот раз денотационное,
...
Чем именно является программа? ◊term{Программа} — это описание вычислений,
...
Программы записываются на некотором ◊term{языке}; определение этого языка
придаёт корректным программам ◊term{смысл}.
Смысл программы — это не~только и
не~столько результаты вычислений, так как они зависят от введённых данных, от
событий во~внешнем мире, от действий других программ.
Смысл программы — это
нечто большее, это суть проводимых вычислений.

Если мы хотим как-то изучать смысл программ и обрабатывать их, то они обязаны
иметь математическое представление.
Возьмём любое преобразование, например,
...
преобразованиях.
А~для этого необходимо связать смысл программ с~математическими
объектами.
Такой подход кажется вполне разумным и полезным: например, если~бы
...
программа.
В~общем, проблема сводится к~отысканию способа построения
математических эквивалентов всех конструкций языка.
То~есть к~формализации его
◊term{семантики}.
...
значение или побочные эффекты.
Но, как и с~любым чёрным ящиком, очень непросто
построить полную теорию его работы, пользуясь лишь подобными наблюдениями.
Если
...
Это не~решает
проблему одним махом, но разделяет её на две проблемы поменьше.
Сначала нам
необходимо описать конструкции языка с~помощью ограниченного числа операций
вычислительной машины определённой архитектуры.
После этого остаётся только
понять, как работает данная машина.
Сама виртуальная машина является формальной
...
Главная проблема — это разработать хорошую виртуальную машину.
Это не~так
легко, как кажется: она должна одновременно подходить для языка, быть простой
в~использовании и тривиальной в~реализации.
На~выбор есть множество вариантов:
стековые машины, регистровые, основанные на деревьях, графах — в~общем, всё,
что душе угодно.
(Здесь мы сначала выбираем язык ассемблера, а затем подгоняем
...
реальной, то подобный способ определения языка называется ◊term{операционной
семантикой}.

◊indexR{денотация!определение}
Но у~данного способа есть врождённый недостаток: ему необходима специальная
вычислительная машина; иными словами, формальная теория вычислений.
Если
в~качестве такой теории взять какую-нибудь простую, всем понятную концепцию, то
можно избавиться от поддержки зоопарка всевозможных машин.
Программа — это,
прежде всего, функция, преобразующая входные данные в~выходные.
В~этом случае
не~требуется сложная машина: математика веками оттачивала такой способ
«исполнения программ», называя его «применением функций».
Таким образом,
...
Его аксиомы настолько
просты, что их толкование не~вызывает никаких разногласий.
Таким образом,
семантику языка программирования можно понимать как способ перевода программ
в~соответствующие денотации.
Этот процесс, между прочим, тоже можно представить
как функцию.
Денотация программы — это выражение ◊${◊lambda}-исчисления
(◊${◊lambda}-терм), представляющее смысл программы.
Вооружившись теорией
...
общая идея — это именно то, что называется ◊term{денотационной семантикой}.

◊indexR{семантика!аксиоматическая}
Стоит упомянуть ещё один способ понимания программ, особо полезный для
доказательства их корректности.
Речь идёт об~◊term{аксиоматической семантике}
Ричарда~Флойда и~Тони~Хоара.
Идея состоит в~том, что для всех конструкций языка
составляются логические утверждения вида ◊${◊{P◊}◊text{◊ii{форма}}◊{Q◊}}.
Данная
формула означает, что если утверждение~◊${P} истинно перед исполнением~◊ii{формы},
то в~результате её исполнения станет истинным~◊${Q}.
В~итоге конструкции языка
сводятся к~набору подобных утверждений-аксиом, из которых выводятся
утверждения-теоремы о~поведении программ.
Такой подход несомненно удобен для
доказательства корректности программ, но, к~сожалению, ничего не~говорит о~том,
как реализовывать вычислитель для языка.
Ни~даже о~том, возможно~ли построить
...
Например, существует ◊term{естественная семантика} ◊cite{kah87}, подобная
денотационной, но с~большим упором на логический вывод свойств программы из
аксиом.
Или ◊term{алгебраическая семантика} ◊cite{ff89}, рассматривающая правила
эквивалентных преобразований программ.


◊section{◊texorpdfstring{Краткий~обзор ◊${◊lambda}-исчисления}%
...
функции, называемой ◊term{интерпретатором} (◊english{valuation function}),
которая переводит корректную программу на языке в~соответствующую денотацию —
элемент множества денотаций.
В~качестве подобного множества мы решили взять
...
абстракциями и аппликациями.
Множество всех доступных переменных будем
обозначать ◊Vset{Переменные}.
Множество всех термов ◊${\lambda}-исчисления будем
обозначать~◊${\lambda}; его можно индуктивно определить следующим образом:

◊[ ◊begin{array}{rll}
  ◊text{◊ii{переменные}:} & ◊forall x ◊in ◊Vset{Переменные}◊colon
                            & x ◊in \lambda                                   ◊◊
  ◊text{◊ii{абстракции}:} & ◊forall x ◊in ◊Vset{Переменные}, M ◊in \lambda◊colon
                            & \lambda x . M ◊in \lambda                       ◊◊
  ◊text{◊ii{аппликации}:} & ◊forall M, N ◊in \lambda◊colon
                          & (M◊ N) ◊in \lambda
◊end{array} ◊]
...
С~помощью ◊${\lambda}-исчисления мы можем определять функции.
Есть даже правило
их применения — ◊term{◊${\beta}-редукция}: в~результате применения
...
записывается: ◊${M[x ◊to N]}.
Это обычная модель подстановки, испокон веков
используемая математикой как само собой разумеющееся.
Именно так проводятся
...
%
◊[ ◊text{◊${\beta}-редукция:} ◊quad (\lambda x . M◊ N) ◊to*{\beta} M[x ◊to N] ◊]

◊indexR{лексическое связывание!в лямбда-исчислении@в~◊${\lambda}-исчислении}
Подстановка ◊${M[x ◊to N]} должна быть достаточно сообразительной, чтобы нечаянно
не~затронуть лишние переменные.
Имеются в~виду свободные переменные, которые
могут появиться при подстановке, если ◊${M} является абстракцией.
Свободные
...
явной.
Говоря более понятным языком, в~◊${\lambda}-исчислении принято лексическое
связывание.
Подобная подстановка определяется следующим образом (скобки отделяют
выражения, где происходят изменения):
%
◊begin{align*}
  &  x[x ◊to N] = N ◊◊
  &  y[x ◊to N] = y ◊quad ◊text{если ◊${x ◊ne y}}                               ◊◊
  &  (\lambda x . M)[x ◊to N] = \lambda x . M                                 ◊◊
  &  (\lambda y . M)[x ◊to N] = \lambda z . ◊big(M[y ◊to z][x ◊to N]◊big)
         ◊quad ◊text{где ◊${x ◊ne y} и ◊${z} не~свободна в~◊${M} и~◊${N}}             ◊◊
  &  (M_1◊ M_2)[x ◊to N] = (M_1[x ◊to N]◊ M_2[x ◊to N])
◊end{align*}
...
абстракцией.
◊${\beta}-редукция позволяет избавиться от редексов.
Если терм
не~содержит редексов (является неприводимым), то говорят, что он находится
в~◊term{нормальной форме.} Термы ◊${\lambda}-исчисления не~обязательно имеют
...
число ◊${\beta}-редукций.
◊term{Стратегией вычислений} называется правило выбора
редекса (если их несколько), который будет редуцирован следующим.
К~сожалению,
есть как хорошие правила, так и плохие.
Пример хорошего правила: редуцировать
самый левый редекс.
Оно не~обязательно ведёт по кратчайшему пути, но
гарантированно достигает нормальной формы (если она вообще существует).
Данное
правило называется~◊term{нормальным} порядком вычислений, он~же ◊term{вызов по
имени}.
Примером плохого правила — именно его использует Scheme "--- является
◊term{аппликативный} порядок вычислений, также известный как ◊term{вызов по
значению} или ◊term{энергичный} порядок (◊english{eager evaluation}).
В~этом
случае функция применяется лишь после вычисления её аргументов.
Рассмотрим
несколько примеров.
Вот терм, который не~имеет нормальной формы:
%
◊[
  (\omega◊ \omega) ◊quad ◊text{где}◊ \omega = \lambda x . (x◊ x)
      ◊quad ◊text{так как}◊ (\omega◊ \omega) ◊to*{\beta} (\omega◊ \omega)
                                             ◊to*{\beta} (\omega◊ \omega)
                                             ◊to*{\beta} ◊dots
◊]
...
◊[
  ◊big((\lambda x . \lambda y . y◊ (\omega◊ \omega))◊ z◊big)
      ◊to*{\beta} (\lambda y . y◊ z) ◊to*{\beta} z
◊]
%
...
запрещает избавляться от них в~теле функций.
Так что можно спокойно передавать
...
имени, даже если их улучшить до ◊term{вызовов по необходимости} (также известных
как ◊term{ленивый} порядок вычислений).
Во-вторых, если плохое правило приводит
к~нормальной форме, то это будет та~же форма, которую мы~бы получили при
использовании хорошего правила.
Поэтому ради эффективности Scheme использует
...
Теперь бессмысленность вычисления
...
◊${\lambda}-исчисление.
Мы могли~бы также добавить функции от этих термов:
например, ◊${2 + 2 ◊to 4}; они называются ◊${◊delta}-правилами.
Однако, подобные
расширения не~являются в~строгом смысле необходимыми, так как числа, булевы
значения, сложение, вычитание, логическое~◊textsc{или} — всё это можно
представить и с~помощью обычных ◊${\lambda}-термов.
Даже списочные структуры
...
не~такая запутанная.
Также важно, что ◊${\lambda}-исчисление обладает при той~же
простоте более гибким понятием эквивалентных программ: два терма эквивалентны,
если эквивалентны их нормальные формы.
По~всем перечисленным причинам именно
