...
которое определяется как (почти) случайное число ◊ic{813}~◊cite{leb05}.
...
Вся проблема возникает из-за того, что ◊ic{begin} обязана вернуть ◊emph{какое-то} значение.
Как и Scheme, определяемый язык может не~придавать какого-либо смысла форме ◊ic{(begin)};
мы можем или допускать такое написание и возвращать любое удобное значение,
или~же не~допускать и считать ошибкой.
Идеальный вариант: вообще не~использовать ◊ic{begin} без аргументов,
...
(Не~следует путать этот объект с~псевдозначением у~неопределённых переменных.)
...
(если только компилятор не~«оптимизирует» «бесполезный»~код до его исчезновения).
...
потому как очевидно, что лучше сначала задать вопрос (с~помощью ◊ic{display}),
