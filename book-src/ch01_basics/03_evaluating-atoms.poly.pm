...
Любая нотация подразумевает определённые условности, поговорим о~таких условностях для записи программ.
...
Как и компилятор, ◊ic{evaluate} начинает свою работу с~синтаксического анализа,
...
Если это символ, то он ссылается на ◊term{переменную}, значение которой хранится в~окружении:
...
И~тут врождённая лень лисперов подсказывает, что в~качестве ключей можно использовать символы.
...
Программы порой содержат ошибки, которые интерпретатору надо как-то обрабатывать.
Большинство Лисп-систем имеет механизмы обработки исключительных ситуаций,
но~у~каждой реализации они свои собственные и не~всегда хорошо переносимы.
...
передавая ей первым аргументом строку.
В~строке находится текстовое описание ошибки,
а~следующие аргументы несут дополнительную информацию о~проблеме.
Системы с~зачаточным механизмом обработки ошибок
в~случае проблем обычно показывают загадочные надписи вроде ◊ic{"Bus~error: core~dumped"} и~умирают.
