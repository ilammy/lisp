...
В~некотором понимании, Лисп является лишь прикладным ◊nobr{◊${\lambda}-исчислением}
...
описывая таким образом случаи, когда может быть сгенерирован высокоэффективный машинный~код.
...
◊footnote{◊ic{if}~не~обязательно принимает условие и ровно две альтернативы.
К~примеру, в~Scheme и ◊(CommonLisp) ◊ic{if}~может принимать как два, так и три аргумента;
