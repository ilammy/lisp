...
Так что между ними присутствуют примерно те~же отношения,
что и между Паскалем (на~котором написана первая реализация~◊(TeX)) и~самой системой~◊(TeX) ◊cite{knu84}.
...
Отличный вариант, так как ◊ic{eq?} в~Scheme сравнивает не~значения, а~адреса.
...
◊term{ложь} это не~◊ic{()} (в~конце концов, это всего лишь пустой список),
и~они оба тем более не~имеют никакого отношения к~символу,
составленному из букв ◊ic{N},~◊ic{I},~◊ic{L}.

Такую позицию занимает Scheme; об~этом всё~же смогли договориться за пару недель до принятия стандарта~IEEE~◊cite{iee91}.

Тепер в~Scheme всё хорошо
