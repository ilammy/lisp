...
Исключительно для удобства, потому что мы о~них ещё даже не~говорили.
...
Эти макросы существенно облегчат наполнение глобального окружения.
Само глобальное окружение является расширением начального окружения~◊ic{env.init}.
...
Заметим, что ◊ic{t} — это переменная в~определяемом Лиспе, а~◊ic{#t} — это значение из Лиспа, используемого для реализации.
...
◊itemize{
  ◊item{
    Отсутствие конфликтов именования:
...
Наиболее простой способ — это вшить значения ◊ic{t} и~◊ic{f} в~вычислитель:
...
при использовании компилятора от функций вроде ◊ic{my-test} ожидается первый вариант — сохранение изначального опрвделения,
...
в~глобальное окружение, так как сейчас у~нас нет иного способа создавать переменные.
По статистике, предлагаемые имена составляют приблизительно 96,7549~% используемых при тестировании свеженаписанных интерпретаторов.
...
(здесь показаны не~все, потому что мне платят не~построчно).
