...
Структура данных, которая связывает переменные с~их значениями, называется ◊term{окружением}~(◊nobr{◊english{environment}}).
Таким образом, функция ◊ic{evaluate}
◊footnote{
  Как вы знаете, «родная» функция-вычислитель Scheme называется ◊ic{eval}.
