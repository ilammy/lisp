...
стоит отметить, что вызов функции при использовании этого метода выходит дороже,
потому что требуется сначала где-то сохранить текущие значения аргументов,
...
а~это может помешать оптимизации хвостовой рекурсии.
...
Новые функции будем обозначать префиксом ◊ic{s.}, чтобы не~путать их с~другими.
...
но~тем не~менее, даже в~◊cite{as85} есть аналогичные ◊ic{put} и~◊ic{get}.
...
◊footnote{
  Эти функции проходят по списку свойств символа (его P-списку, от~◊english{property})
