...
функция~◊ic{invoke} должна лишь проверить, действительно~ли её первый аргумент является функцией:
...
Проще некуда.
Даже может возникнуть вопрос: зачем вообще нужна отдельная функция,
...
Также у~нас появляется новый тип ошибок, возникающих при попытке вызвать невызываемое.
...
Вычислять аргументы слева направо кажется логичным для людей, читающих слева направо.
Это и запрограммировать легче, в~итоге порядок прост и понятен.
Сложности возникают только у~компилятора, потому что если ему захочется изменить этот порядок
...
◊enumerate{
  ...
  ◊item{
    ...
    ◊footnote*{
      ...
      но это не~имеет отношения к~механизму вызова.
    }
  }
}
...
и~должен в~случае необходимости использовать ◊ic{begin}, чтобы задать явный порядок.
...
Ошибки, возникающие в~подобных случаях, очень сложно отлавливать.
