...
Теме функций было посвящено много исследований, изобретено порядочно подходов к~реализации,
...
Мы~поговорим о~том, что называется ◊Lisp-1 и~◊Lisp-2,
...
функция делает только то, для чего она запрограммирована.
К~примеру, с~помощью функций даже реализуют объекты, имеющие поля и методы.
◊seeCite{ar88}
...
Изначально ◊|LISP-1.5|~◊seeCite{mae+62} не~считал функции типом данных.
...
а~его потомок ◊|CommonLisp|~◊seeCite{ste90} лишь недавно получил поддержку функций как объектов первого класса.
...
функции принимают их как аргументы и возвращают как результат вычислений,
