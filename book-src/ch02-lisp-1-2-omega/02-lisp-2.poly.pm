...
◊section[#:label "lisp1-2-omega/sect:lisp2" #:alt "Lisp-2"]{◊(Lisp2)}
...
Так как теперь у~нас два окружения и два вычислителя, то мы назовём это ◊(Lisp2)~◊cite{sg93}.
...
так как тех~же результатов можно добиться и в~◊(Lisp1) с~помощью небольшого предварительного анализа программ.
Есть только один действительно приятный момент:
◊(Lisp2) чуть-чуть быстрее, так как мы можем быть уверены в~том, что любое имя из~◊ic{fenv} связано с~функцией и ни~с~чем иным,
...
Но~в~◊(Lisp2) такая программа некорректна,
