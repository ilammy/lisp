...
Scheme является ◊|Lisp-1|, на нём приятно программировать и ему легко обучать, так как процесс вычислений прост и последователен.
◊|Lisp-2|~же более сложен, потому как существование двух миров требует использования особых форм для перехода между ними.
◊CommonLisp является не~совсем ◊Lisp-2, так как имеет множество других пространств имён:
...
Внутреннее представление программ может быть ◊|Lisp-2|-языком, который удаётся лучше компилировать.
...
очередного ◊|Lisp-2|-языка, похожего на ◊|CommonLisp|.
...
Ещё одно важное, по мнению ◊cite{gp88}, практическое различие между ◊Lisp-1 и ◊Lisp-2 состоит в~читабельности.
