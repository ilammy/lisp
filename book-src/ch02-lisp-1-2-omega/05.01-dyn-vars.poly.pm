...
◊indexE{Lisp-3@◊Lisp-3}
◊indexR{Лисп!Lisp-3@◊Lisp-3}
...
Новый интерпретатор — назовём его ◊Lisp-3 — будет использовать функции с~префиксом~◊ic{df.}
Вот его код.
...
Теперь ещё появилось окружение с~динамическими переменными, существующими в~момент вызова.
Это окружение не~может быть захвачено и сохранено в~замыкании —
значения динамических переменных всегда извлекаются из~текущего динамического окружения.
...
Это очень важно, так как в~◊Lisp-3 поведение функции определяется не~только значениями локальных переменных,
...
но такой подход требует нежелательных присваиваний для использования правильного обработчика в~правильное время.
...
у~◊ic{dynamic-let} есть несколько существенных преимуществ:
...
◊noindent
Поэтому ◊ic{dynamic-let} идеально подходит для временной установки функций-обработчиков ошибок.
...
◊footnote{
  ...
  в~том числе к~равенству возможностей разработчиков языка и программистов.
...
По~умолчанию функции вывода вроде ◊ic{print} и ◊ic{write} используют порт, хранящийся в~переменной ◊ic{*standard-output*}.
◊footnote{По~соглашению, имена динамических переменных выделяются звёздочками.}
Функция ◊ic{with-output-to-file} реализуется
◊footnote{Разумеется, не~в~◊|CommonLisp|, а~в~нашем~◊|Lisp-3|.}
следующим образом:
