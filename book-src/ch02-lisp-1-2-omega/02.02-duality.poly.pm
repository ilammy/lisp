...
Обобщая данные затруднения, можно сказать,
...
В~◊(Lisp2) ◊ic{funcall} используется для вызова динамически вычисляемых функций.
...
обе принимают первым аргументом функцию, а~за ним — необходимые аргументы.
Разница между ними лишь в~том, что ◊ic{funcall} статически знает количество передаваемых аргументов.
...
ведь ◊ic{evaluate-application} работает со~значениями и вызывает их как функции;
...
◊code:lisp{
(if ◊ii{условие} (+ 3 4) (* 3 4)) ◊(eq) (funcall (if ◊ii{условие} (function +) (function *)) 3 4)
}
