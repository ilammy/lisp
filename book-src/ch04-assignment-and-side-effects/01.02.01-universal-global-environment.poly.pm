...
◊seeCite{que95}
В~таком мире с~каждым именем связана одна и~только одна переменная.
Определение переменной здесь не~имеет смысла — все переменные уже определены.
Изменение переменной тоже не~вызывает проблем: мы~уверены, что изменяемая переменная существует.
Соответственно, форма ◊ic{define} фактически эквивалентна ◊ic{set!}
и,~как~следствие, можно безболезненно «определять» переменную несколько~раз.
...
Видишь переменную? Она~есть — а~значения~нет.
...
2)~любая~переменная может быть переопределена — то~есть нельзя полагаться на неизменность значения переменных.
