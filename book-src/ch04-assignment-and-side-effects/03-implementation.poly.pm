...
а~в~ответ на~◊ic{type} объект сообщает свой~тип.
...
Звучит просто и понятно, но~практическая реализация оказывается не~настолько простой.
...
а~она передаст её дальше другим функциям, возможно внеся некоторые изменения.
При таком подходе вычисления описывается четвёркой из ◊term{выражения}, ◊term{окружения}, ◊term{продолжения} и~◊term{памяти}.
В~программах будем их коротко называть ◊ic{e},~◊ic{r},~◊ic{k},~◊ic{s}.
