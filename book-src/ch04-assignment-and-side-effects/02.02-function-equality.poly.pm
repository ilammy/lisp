...
Сравнение функций может показаться безнадёжным занятием.
...
К~сожалению, сравнение функций — это в~общем случае алгоритмически неразрешимая задача.
Таким образом, мы вынуждены либо отказаться от идеи сравнения функций в~принципе,
...
однако порой бывает очень легко понять, когда некоторые функции не~могут быть эквивалентными
...
Выражение ◊nobr{◊ic{(eqv? car cdr)}} очевидно возвращает~◊ic{#f},
...
как~◊ic{(eqv? (lambda~(x)~(◊ii{car}~x)) (lambda~(x)~(◊ii{car}~x)))}.
...
так~что даже если её аргументы эквивалентны, то~в~результате получаются очевидно различные объекты —
ведь~они размещаются в~различных участках памяти.
...
◊trnote{
  ...
  а~лишь для тех, где~◊${a \geqslant b}.
}
...
Именно так и поступает ◊ic{eqv?}, считая функции с~одинаковыми адресами эквивалентными.
...
Как~видим, сравнение функций вызывает множество вопросов, на которые нельзя дать однозначные ответы.
...
Существует множество вариантов реализации сравнения с~различными свойствами.
