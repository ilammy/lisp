...
Все~значения будут основываться на функции, которая распознаёт как минимум два~сообщения:
◊; TODO: форматирование списков: 1) или 1. -- заглавная или строчная первая буква -- нужны ли ; и/или . в конце
◊enumerate{
...
Таким~образом, все определения значений следуют шаблону:
...
◊indexE{name@◊ic{'name}}
Символы имеют имена, которые они назовут, если их вежливо попросить с~помощью ◊ic{name}.
...
Однако у~каждого элемента массива очевидно есть собственный адрес в~памяти,
то~есть точечные~пары — это~пары адресов.

Для удобства мы также определим функцию, выделяющую в~памяти списки.
Она принимает список значений, которые необходимо разместить в~памяти, собственно память, и~наконец продолжение,
которому передаётся созданный список вместе с~новым состоянием~памяти.
