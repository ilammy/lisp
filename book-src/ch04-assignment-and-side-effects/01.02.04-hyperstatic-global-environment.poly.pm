...
Здесь фраза «замыкание сохраняет свободные переменные из окружения определения» понимается буквально.
...
например, необходимость повторять все последующие определения после исправления~ошибки в~одном из~предыдущих.
