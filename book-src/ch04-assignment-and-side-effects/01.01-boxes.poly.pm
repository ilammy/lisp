...
содержащие множество пар ◊nobr{«переменная — значение»}
(если~быть точным, то~◊emph{список} точечных~пар).
При~обращении к~переменной мы ищем соответствующую точечную пару,
а~присваивание переменной записывает новое значение в~◊ic{cdr} найденной точечной~пары.
В~данном случае точечные пары являются реализацией привязок — одним из способов реализации.
...
◊noindent
Можно было~бы реализовать их и без замыканий, используя точечные пары напрямую:
...
◊; TODO: надо сделать как-то так, чтобы вся фраза "упражение А.Б" была ссылкой
...
достаточно посмотреть на соответствующую связывающую форму и проверить, упакована~ли переменная в~коробку.
...
◊; осло, коробочки должні біть одинаковіе по вісоте
...
Однако, помимо преимуществ, использование коробок имеет и~свои специфичные недостатки.
...
что порой оказывается полезным, например, для реализации модулей,
а~порой приводит к~неожиданному поведению программ.
...
Также не~стоит забывать, что в~реальном компьютере значения переменных хранятся в~памяти, по~определённым адресам.
