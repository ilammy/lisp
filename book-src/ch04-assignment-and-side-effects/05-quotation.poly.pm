...
Вы уже наверное заметили, что в~этот раз мы как-то обошли стороной цитирование.
...
Но,~к~сожалению, точный смысл этого выражения несколько отличается от~того, который обычно подразумевается в~Лиспе и~Scheme.
Рассмотрим выражение ◊nobr{◊ic{(quote (a . b))}}.
По~определению, оно ◊emph{в~точности} эквивалентно ◊nobr{◊ic{(cons 'a 'b)}}.
◊footnote{Ещё~точнее: ◊nobr{◊ic{'(a . b) ◊(eq) `(,'a . ,'b)}}.}
...
Другим подходом к~реализации цитирования является трансформация программ:
...
На~языке ассемблера или~Си строки представляются непосредственно,
...
И~если затем распечатать определение ◊ic{vowel<=}
...
Аналогичным образом возможно добавлять элементы в~список ◊ic{vowels} или изменять~их.
Правда, такие трюки допустимо проделывать только в~интерпретаторе —
...
Подобная оптимизация очевидно влияет на цитаты с~общими подвыражениями и~поведение~◊ic{eq?}.
Есть простой способ избежать всех этих проблем — запретить изменять цитаты.
Именно так и поступают Scheme и~◊|CommonLisp|,
где изменение значения цитат приводит к~неопределённому поведению.
...
Аналогичным образом можно реализовать неизменяемые строки (как~это сделано, например, в~Mesa),
...
Рассмотрим синтаксис ◊ic{#.◊ii{выражение}}, вместо которого подставляется значение ◊ii{выражения} —
...
◊; TODO: когда ◊seeCite используется прямо в тексте, надо ли перед ним ставить неразрывный пробел? это дублирует пробелы, где-то ты уже наступил на эти грабли
◊noindent
то~переменная ◊ic{bar} будет содержать бесконечный список из чередующихся нулей и~единиц.
С~помощью~◊ic{#.} мы процитировали пару, чей ◊ic{cddr} является ей самой.
В~CLtL2 ◊seeCite{ste90} прямым текстом написано, что ◊ic{#.} существует именно для ввода таких неудобных значений.
Если мы захотим реализовать такую возможность у~себя,
...
Хорошая~новость: в~Scheme так писать нельзя, ◊ic{read} не~программируется и макросимволы не~поддерживаются.
...
◊; TODO: поведение сносок, которые следуют сразу за знаком препинания
◊footnote{
